...
void main() {
  ...
  late SaveRefreshCompletedAlertsEnabledUseCase useCase;

  setUp(() {
    mockRepository = MockIRefreshSettingsRepository();
    useCase = SaveRefreshCompletedAlertsEnabledUseCase(mockRepository);
  });

  group(
    'Usecase SaveRefreshCompletedAlertsEnabledUseCase returns the correct value',
    () {
      test('returns Right when repository succeeds', () async {
        when(
          () => mockRepository.saveRefreshCompletedAlertsEnabled(true),
        ).thenAnswer(
          (_) async =>
              Right(buildRefreshSettings(refreshCompletedAlertsEnabled: true)),
        );
      ...
      test('returns Left when repository fails', () async {
        ...
        when(
          () => mockRepository.saveRefreshCompletedAlertsEnabled(false),
